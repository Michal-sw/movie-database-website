...
const Stars = (props) => {
  ...
  const onSaveRating = (index) => {
    if (vote !== true) {
      ...
      setVote(true)
      axios
        .patch(`http://localhost:3000/movie/${props.movieId}/rate?score=${index}`)
        .then(() => console.log(`Ocena zmieniona na ${index} dla ${props.movieId} id`))
        .catch(() => console.log("rating change error"))
    }
